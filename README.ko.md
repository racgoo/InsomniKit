...
| **Lid-Closed 모드** | 옵트인: lid 닫아도 시스템이 돌게 함. 배터리 상태에서도. |
...
```
  ...
  ▸ 배터리 상태? Lid-Closed 모드가 이동 중에도 작업 살려둠
...
| 이런 상황                                  | 이렇게                                                                   |
| ------------------------------------------ | ------------------------------------------------------------------------ |
| 자리 비운 사이 AI 에이전트 계속 돌리기     | **Enable** → **Duration → ∞** → lid 닫기 (충전기 꽂거나 Lid-Closed 모드) |
...
| **배터리** + InsomniKit 켜짐        |    **잠**      | macOS가 lid 닫힐 때 강제 sleep. `caffeinate -s`는 AC 전용. 메뉴가 경고함: `⚠︎ Lid-close sleeps on battery`. |
| **AC + 외장 디스플레이**            | 안 잠 (clamshell) | 네이티브 clamshell 모드 — InsomniKit 없이도 됨.                         |

**요약** — AC면 그냥 lid 닫으세요, 작업은 계속됩니다. 배터리면 먼저 충전기 꽂으세요 — *또는* Lid-Closed 모드를 켜세요.

<details>
<summary><b>Lid-Closed 모드 — 배터리 + lid 닫음용 (고급, 옵트인)</b></summary>

<br>

**배터리 상태에서** lid 닫고도 시스템을 깨워둬야 한다면, InsomniKit이 `pmset -c disablesleep 1`을 대신 켜줍니다. 메뉴 → **Lid-Closed Mode → Turn on…**
