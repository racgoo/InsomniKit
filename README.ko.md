...
작은 **macOS 전용** 메뉴바 유틸리티 — 트레이 중심, 필요하면 윈도우와 데스크톱 위젯도. 텔레메트리 없음. 군더더기 없음.
...
2. 자동으로 **앱을 실행**합니다.
3. 작은 달 아이콘이 **메뉴바 우측 상단** (Wi-Fi, 배터리, 시계 옆)에 뜹니다. 기본은 **트레이 중심**이라 평소엔 Dock 아이콘이 없습니다. 전체 창이 필요하면 앱을 클릭하거나 트레이 메뉴에서 열면 됩니다 — 윈도우와 데스크톱 위젯을 제공합니다.
...
| **한눈에 들어오는 트레이 아이콘** | 초승달 — 활성 시 채워짐, 비활성 시 윤곽선. **Stay Awake When Closed** 켜져있으면 작은 패들록 배지. 활성 중일 땐 부드러운 호흡 애니메이션 (24-frame sinusoidal); 메뉴에서 언제든 끌 수 있음. |
| **다국어 UI** | 7개 언어 — **English · 한국어 · 日本語 · 中文 · Español · Deutsch · Français** — 시스템 로케일 자동 선택, 또는 **🌐 Language** 서브메뉴에서 수동 고정. |
| **숨김 모드** | 메뉴 클릭 한 번으로 트레이 아이콘 완전히 숨김; 앱은 백그라운드에서 계속 동작. Spotlight에서 InsomniKit 다시 실행하면 복원. |
| **확실한 정리** | 크래시, 강제종료, `kill -9` — `caffeinate` 좀비도, `pmset` 잔여 상태도 안 남음. 절대로. |
| **다 기억함** | 시간, 배터리 기준, Launch at Login, 언어 선택, 애니메이션 토글 — 다음 실행 때 그대로 복원. |
